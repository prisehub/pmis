...
class AccountAnalyticLinePlan(models.Model):
    ...
    amount = fields.Float(string="Amount", store=True, compute="_compute_total_amount")
    ...
    user_id = fields.Many2one(
        comodel_name="res.users", string="User", default=_default_user
    )
    ...
    journal_id = fields.Many2one(
        ...
        ondelete="restrict",
        index=True,
        default=lambda self: self._context["journal_id"]
    ...
    version_id = fields.Many2one(
        ...
        required=True,
        ondelete="cascade",
        default=lambda s: s.env["account.analytic.plan.version"].search(
    ...
    def _set_unit_price(self):
        ...
        if journal.type != "sale":
            for line in self:
                ...
                if pricelist_id and line.partner_id and line.product_uom_id:
