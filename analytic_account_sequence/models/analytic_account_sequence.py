...
class AnalyticAccountSequence(models.Model):
    ...
    prefix = fields.Char(
        "Prefix", help="Prefix value of the record for " "the sequence"
    )
    suffix = fields.Char(
        "Suffix", help="Suffix value of the record for " "the sequence"
    )
    number_next = fields.Integer(
        "Next Number", required=True, default=1, help="Next number of this sequence"
    )
    ...
    def create(self, values):
        ...
        if values.implementation == "standard":
            values._create_sequence(values.number_increment, values.number_next)
        return values
    ...
    def _next(self):
        ...
        if not force_company:
            force_company = self.env["res.users"].browse(self._uid).company_id.id
        preferred_sequences = [
            s for s in self if s.company_id and s.company_id[0] == force_company
        ]
        ...
        try:
            ...
            raise UserError(
                _("Invalid prefix or suffix for sequence '%s'") % (seq.get("name"))
            )
        return (
            interpolated_prefix
    ...
    def next_by_id(self, sequence_id):
        ...
        ids = self.search(
            ["&", ("id", "=", sequence_id), ("company_id", "in", company_ids.ids)]
        )
        return ids._next()
